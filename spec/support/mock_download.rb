shared_context 'mock download' do
  before do
    ...
    stub_request(:get, 'http://example.com/api/v1/documents/404/download?access_token=token').
      to_return({ status: 201, body: nil })

    stub_request(:get, 'http://example.com/api/v1/documents/500/download?access_token=token').
      to_return({ status: 500, body: 'buggy server'})
  end
end
