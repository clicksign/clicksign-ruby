...
Before do
  ...
  # stub_request(:post, 'http://example.com/documents/coffee').
  stub_request(:get, 'http://example.com/documents/coffee?access_token=token').
    # with(query: { access_token: 'token' }).
    to_return(status: 200, body: { document: { key: '17' }}.to_json)

  stub_request(:get, 'http://example.com/documents/invalid?access_token=token').
    # with(query: { access_token: 'token' }).
